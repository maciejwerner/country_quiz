...
const cardQuestion = (country) => {
  ...
  const template = (randomNr % 2 === 0) 
    ?
      `<div class="question-box--header">
        <img src="${ country.flag }" alt="Country flag - ${ country.name }" class="img img-flag">
...
const displayQuestionCard = async () => {
  ...
  questionBox.classList.contains("quiz-winner") ?  questionBox.classList.remove("quiz-winner") : '';
...
const checkAnswer = (e, countries, answerBtns) => {
  ...
  (gameSumary.allAnswers === 4 || gameSumary.correctAnswers === 4) ? '' : nextBtn.classList.remove("disabled");
